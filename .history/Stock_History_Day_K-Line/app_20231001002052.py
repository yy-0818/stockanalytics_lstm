...
# # 获取用户输入参数
# start_date = st.sidebar.date_input("选择开始日期", min_value=load_stock_data['日期'].min(), max_value=load_stock_data['日期'].max())
# end_date = st.sidebar.date_input("选择结束日期", min_value=load_stock_data['日期'].min(), max_value=load_stock_data['日期'].max())
# prediction_days = st.sidebar.number_input("预测天数", min_value=1, max_value=365, value=30)

# # 处理用户输入参数并准备数据
# start_index = load_stock_data[load_stock_data['日期'] == start_date].index[0]
# end_index = load_stock_data[load_stock_data['日期'] == end_date].index[0]
# input_data = load_stock_data['收盘'][start_index:end_index+1].values
...
# 在侧边栏中添加一些参数输入框，如选择开始日期、结束日期、预测天数等
start_date = st.sidebar.date_input("选择开始日期", min_value=load_stock_data['日期'].min(), max_value=load_stock_data['日期'].max())
end_date = st.sidebar.date_input("选择结束日期", min_value=load_stock_data['日期'].min(), max_value=load_stock_data['日期'].max())
prediction_days = st.sidebar.number_input("预测天数", min_value=1, max_value=365, value=30)
