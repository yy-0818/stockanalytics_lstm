...
# 将默认值设置为合理的日期范围内
default_start_date = datetime(2023, 9, 1)
default_end_date = datetime(2023, 9, 29)
...
# 获取用户输入参数
start_date = st.sidebar.date_input("选择开始日期", min_value=stock_data['日期'].min(), max_value=stock_data['日期'].max())
end_date = st.sidebar.date_input("选择结束日期", min_value=stock_data['日期'].min(), max_value=stock_data['日期'].max())
