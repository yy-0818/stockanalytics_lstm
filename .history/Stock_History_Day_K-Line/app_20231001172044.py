...
def user_input_features():
    ...
    hashtag_key = 'hashtag_input'
    hashtag = st.sidebar.text_input('输入hashtag', value='', key=hashtag_key)
    button = st.sidebar.button('Get Data')

    model_key = 'model_selection'
    model_name = st.sidebar.selectbox("选择模型", ('RNN', 'LSTM'), key=model_key)

    a9 = st.sidebar.selectbox("Agent Status?", ('Happy','Sad','Normal'))
    output = [model_name,a9]
    return output
