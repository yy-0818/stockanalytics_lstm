...
def load_prediction_model():
    model = load_model(r'.\Stock_History_Day_K-Line\Model\RNN.h5')  # 替换为你的模型文件
    return model
...
def main():
    ...
    if button:
        # 加载选择的预测模型
        if model_name == 'RNN':
            model = load_model(r'.\Stock_History_Day_K-Line\Model\RNN.h5')
            st.success("RNN模型加载成功")
        elif model_name == 'LSTM':
            model = load_model(r'.\Stock_History_Day_K-Line\Model\LSTM.h5')
