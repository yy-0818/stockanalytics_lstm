...
# 创建Streamlit应用程序
st.markdown("<h1 style='text-align: center; color: black;'>基于 LSTM 模型股票市场预测分析📈</h1>", unsafe_allow_html=True)
...
def main():
    ...
    st.title("`{model_name}`模型预测")
    st.write("股票名称：", st.__name__)
    st.write("股票代码：", st.__code__)
    st.write("股票价格预测")
    # 创建一个新的Matplotlib图表
    fig, ax = plt.subplots(figsize=(16, 6))
