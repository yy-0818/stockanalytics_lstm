...
def main():
    ...
    # 添加第一个线条
    fig.add_trace(go.Scatter(x=len(stock_data), y=list(X_test_pred_price), mode='lines', name='prediction'))

    # 添加第二个线条
    fig.add_trace(go.Scatter(x=len(stock_data), y=list(Y_test), mode='lines', name='val'))

    # 设置图表标题和轴标签
    fig.update_layout(
        title='Comparison of X_test_pred_price and Y_test',
