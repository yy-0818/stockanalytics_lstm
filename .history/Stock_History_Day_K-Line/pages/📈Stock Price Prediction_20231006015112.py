...
def main():
    ...
    if stock_model_n in stock_model:
        ...
        ax.set_ylabel(f"{stock_name} Time Price")
        ax.legend()
        ax.grid(True)
        ...
        # 添加数据互动 - 选择数据点以查看详细信息
        data_point = st.slider('选择数据点', min_value=0, max_value=len(Y_test)-1)
        st.write(f'日期: {selected_stock_df.iloc[data_point]["Date"]}')
        st.write(f'开盘价: {selected_stock_df.iloc[data_point]["Open"]}')
        st.write(f'收盘价: {selected_stock_df.iloc[data_point]["Close"]}')
        st.write(f'最高价: {selected_stock_df.iloc[data_point]["High"]}')
        st.write(f'最低价: {selected_stock_df.iloc[data_point]["Low"]}')
    else:
