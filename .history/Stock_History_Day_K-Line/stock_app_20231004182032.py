...
def interactive_galaxies(df):
    ...
    stock_df = st.sidebar.selectbox('选择数据集', ('stock_1', 'stock_2','stock_3'))
    st.sidebar.write('当前数据集为：', stock_df)
    if stock_df == 'stock_1':
        see_data = st.expander('查看原始数据 \ View the raw data 👉')
        with see_data:
            st.dataframe(data=moutai_stock.reset_index(drop=True))
    if stock_df == 'stock_2':
        see_data = st.expander('查看原始数据 \ View the raw data 👉')
        with see_data:
            st.dataframe(data=aapl_stock.reset_index(drop=True))
    if stock_df == 'stock_3':
        see_data = st.expander('查看原始数据 \ View the raw data 👉')
        with see_data:
            st.dataframe(data=tencent_stock.reset_index(drop=True))
