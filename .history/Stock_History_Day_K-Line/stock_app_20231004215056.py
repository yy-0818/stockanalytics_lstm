...
@st.cache_data 
def load_model():
    model_locs = [os.getcwd() + '\Stock_History_Day_K-Line\model_{}.csv'.format(n) for n in range(1, 4)]
    models = [pd.read_csv(model_loc) for model_loc in model_locs]
    return models

@st.cache_data 
def load_data():
    df_locs = [os.getcwd() + '\Stock_History_Day_K-Line\stock_{}.csv'.format(n) for n in range(1, 4)]
...
def interactive_galaxies(df):
    # 创建一个字典，将数据集名称映射到数据框
    stock_data = {
        'stock_1': moutai_stock,
        'stock_2': aapl_stock,
        'stock_3': tencent_stock
    }
    stock_model = {}
    with st.sidebar.expander("📊 数据分布", expanded=False):
        ...
        stock_model = st.sidebar.selectbox('选择数据集', list(stock_model.keys()))

    # 如果用户选择了特定的数据集，则显示对应的原始数据
    if stock_df in stock_data:
        see_data = st.expander('查看原始数据 \ View the raw data 👉')
        with see_data:
            st.dataframe(data=stock_data[stock_df].reset_index(drop=True))
    else:
