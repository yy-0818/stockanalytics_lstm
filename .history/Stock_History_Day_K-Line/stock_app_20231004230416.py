...
def interactive_galaxies(df):
    ...
    # Use the selected stock data DataFrame
    selected_stock_df = stock_data[stock_df]  # Use the selected stock data

    jointplot = sns.jointplot(x='开盘', y='收盘', data=stock_data[stock_df], kind='scatter')  # Use selected_stock_df
    jointplot.set_axis_labels(stock_data[stock_df]['股票名称'].iloc[0], fontsize=12)
