...
def tell_me_more():
    ...
    st.markdown("""
        ...
        循环神经网络是一种对序列数据进行建模的神经网络，它像一个循环动态系统，在该结构中当前的 输出会流入下一步的输入中，为下一次输出做出贡献。其主要形式是该结构有个循环结构会保留前一次 循环的输出结果并作为下一次循环输人的一部分输入。下图为循环神经网络结构图。
        
      """)

    st.image(r'Stock_History_Day_K-Line\images\rnn.jpg', width=500,align='center')
