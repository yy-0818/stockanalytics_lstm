...
def tell_me_more():
    ...
    st.markdown(r"""
    其中 $h_{t-1}$ 表示的是上一个 cell 的输出， $C_{t-1}$ 表示的是当前细胞的输入。σ 表示 sigmod 函数。
    """)
    st.markdown(
    r"""
    输入门（Input Gate），下一步是决定让多少新的信息加入到 cell 状态中来。实现这个需要包括两个步骤：首先，
一个叫做“input gate layer”的 sigmoid 层决定哪些信息需要更新；一个 tanh 层生成一个向量，也就是备
选的用来更新的内容。在下一步，我们把这两部分联合起来，对 cell 的状态进行一个更新：
    ...
    st.markdown("""
    输出门（Output Gate）:最终，我们需要确定输出什么值。这个输出将会基于我们的细胞状态，但是也是一个过滤
后的版本。首先，我们运行一个 sigmoid 层来确定细胞状态的哪个部分将输出出去。接着，我们把细胞
状态通过 tanh 进行处理(得到一个在−1 到 1 之间的值)并将它和 sigmoid 门的输出相乘，最终我们仅仅会
输出我们确定输出的那部分：
    """)
