...
def tell_me_more():
    ...
    st.markdown("""
        **1. RNN 神经网络模型**
        ...
        &emsp;循环神经网络是一种对序列数据进行建模的神经网络，它像一个循环动态系统，在该结构中当前的 输出会流入下一步的输入中，为下一次输出做出贡献。其主要形式是该结构有个循环结构会保留前一次 循环的输出结果并作为下一次循环输人的一部分输入。
    ...
    # """)
    st.markdown(r"""
    &emsp;遗忘门（Forget Gate）,在 LSTM 中的第一步是决定我们会从细胞状态中丢弃什么信息。这个决定通过一个称为遗忘门层完成。该门会读取 $\boldsymbol{h_{t-1}}$和 $\boldsymbol{x_(t)}$ ，输出一个在 0 到 1 之间的数值给每个细胞状态 $\boldsymbol{C_{t-1}}$ 中的数字。1表示“完全保留”，0 表示“完全舍弃”:
    """)
    st.latex(
    r"""
